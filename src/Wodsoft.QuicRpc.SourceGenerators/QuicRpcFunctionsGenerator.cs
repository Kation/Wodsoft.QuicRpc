...
{
    [Generator]
    public class QuicRpcFunctionsGenerator : ISourceGenerator
    {
        public void Execute(GeneratorExecutionContext context)
        {
            if (context.SyntaxReceiver is QuicRpcFunctionsSyntaxReceiver receiver)
            {
                foreach (var classSyntax in receiver.Functions)
                {
                    ...
                    var functionAttribute = classType.GetAttributes().FirstOrDefault(t => SyntaxHelper.IsSameFullName(t.AttributeClass, "Wodsoft.QuicRpc.QuicRpcFunctionAttribute"));
        ...
        {
            context.RegisterForSyntaxNotifications(() => new QuicRpcFunctionsSyntaxReceiver());
        }
